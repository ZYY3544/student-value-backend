...
### 严格按以下格式输出（3个段落，每段格式完全一致）：

**岗位匹配总结**
- 你的月薪估值定位在 {salary_range}，……（说明与{job_title}岗位的匹配情况）
- ……（从赛道适配性角度，说明经历处于什么位置）
- ……（补充一个匹配度相关的要点）

**你的核心亮点**
- ……（从简历中提取第1个亮眼经历，引用原文）
- ……（第2个亮眼经历，说明为什么和目标岗位匹配）
- ……（体现了什么能力，为什么在这个赛道上有优势）

**可以加强的方向**
- ……（指出哪些方面加强后匹配度会更高）
- ……（给出具体的努力方向）
- ……（鼓励性收尾，语气是"提升空间"而非"缺点"）

## 输出要求

1. **总字数**：300-400字
2. **结构**：严格3个段落，每段第一行是 **加粗标题**（独占一行，标题后不跟任何正文），然后换行写 bullet points
3. **段内格式**：每个要点以 `- ` 开头（短横线+空格），每个要点 1-2 句话，每段 2-3 个要点
4. **引用原文**：关键段落引用 1-2 处简历原文（用引号标注）
5. **语气**：像朋友在帮你分析，专业但不端着，通俗易懂
6. **关键词高亮**：对每段中最重要的 2-3 个关键词或短语，用 **双星号** 包裹
7. **禁止**：
