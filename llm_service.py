...
LLM 服务模块
===========================================
支持 DeepSeek / AWS Bedrock Sonnet / GLM 等多种后端
...
class LLMService:
    ...
    支持两种初始化方式：
    1. 传统方式：传入 api_key，自动创建 DeepSeek 客户端
    2. 外部客户端：传入已创建的 client（如 BedrockOpenAIClient），直接使用
    """
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "deepseek-chat",
        ...
        Args:
            api_key: DeepSeek API Key（传统模式）
            model: 模型名称
            client: 外部客户端（如 BedrockOpenAIClient），传入后忽略 api_key
        """
        self.model = model

        if client is not None:
            # 外部客户端模式（Sonnet/GLM 等）
            self.client = client
            self.api_key = None
        ...
        if not self.api_key:
            raise ValueError("未找到 API Key！请在 .env 中设置 DEEPSEEK_API_KEY。")
...
{eval_text}"""
        ...
        try:
            ...
            # response_format 仅 DeepSeek chat 模型支持
            # Sonnet (Bedrock) 和 reasoner 不支持此参数
            is_deepseek = hasattr(self.client, '_base_url') or (self.api_key and 'deepseek' in str(getattr(self.client, '_base_url', '')).lower())
            ...
            # CV 模式下，仅对 DeepSeek chat 模型进行 +0.5 档调整（补偿保守倾向）
            # Sonnet/GLM 等模型不需要此调整
            is_deepseek_chat = (self.api_key and 'reasoner' not in self.model.lower()
