...
### v1.1 — 2026-03-04（稳定性与体验优化）

#### Bug 修复

| # | 问题 | 修复内容 |
|---|---|---|
| 1 | **OptimizeAgent 消息拼接 bug** | `_build_messages` 中 `recent_messages` 可能已包含当前用户消息，导致重复发送。重写拼接逻辑：先排除末尾重复消息，上下文作为独立 user message 注入，确保 user/assistant 交替正确 |
| 2 | **上传简历时聊天 Agent 报"缺少简历内容"** | 用户上传文件时 `inputData.resumeText` 为空（内容在 `resumeFile` base64 中）。修复：后端评估接口返回解析后的 `resumeText`，前端存下来传给 ChatWidget |

#### 性能优化

| # | 问题 | 修复内容 |
|---|---|---|
| 3 | **`start_session` 开场阻塞** | 简历拆分（`split_resume_sections`）和诊断生成串行执行，用户等待时间长。修复：简历拆分改为后台线程（`threading.Thread(daemon=True)`），不阻塞诊断开场白生成 |
| 4 | **`_post_chat_processing` 每轮同步阻塞** | 记忆提取 + 历史压缩在流式输出结束后同步执行（1-2 次额外 LLM 调用），用户要等后处理完成才能发下一条。修复：后处理整体放入后台线程执行 |

#### 功能增强

| # | 问题 | 修复内容 |
|---|---|---|
| 5 | **阶段转换误触发** | 原关键词匹配过于简单，"谢谢你的建议，帮我继续改"会误触发总结。修复：增加"继续优化"信号词（优先级高于总结关键词），扩充关键词列表（"先这样"、"可以了"、"OK了"等），长消息中偶然命中不触发 |
| 6 | **总结阶段无法回退** | 一旦进入 summary 阶段，用户无法再回到优化。修复：summary 阶段检测到继续优化意图时自动回退到 optimizing |
| 7 | **前端会话过期无恢复** | Render 重启后内存会话丢失，用户看到"会话不存在"。修复：前端检测到 404 时自动重新初始化会话并提示"之前的会话已过期，已为你重新开启对话" |
| 8 | **前端 markdown 渲染不完整** | 原 `formatContent` 只处理 `**加粗**`。修复：增加有序/无序列表识别和渲染（`•` 和 `1.` 前缀高亮） |
| 9 | **无消息长度限制** | 用户可发任意长度消息。修复：后端截断超过 2000 字符的消息，前端输入框同步限制 |

#### 改动文件

| 文件 | 改动类型 |
|---|---|
| `chat_agent.py` | 阶段转换逻辑重写、后处理异步化、开场拆分异步化 |
| `multi_agent.py` | OptimizeAgent `_build_messages` 重写 |
| `mini_api.py` | 评估接口返回 `resumeText`、消息长度限制 |
| `frontend-components/ChatWidget.tsx` | markdown 渲染增强、会话恢复、输入长度限制、错误提示中文化 |
